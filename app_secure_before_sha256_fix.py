...
import sqlite3, secrets, threading, socket, urllib.parse, html, os, traceback
...
def patient(t):
    ...
    if request.method=='POST':
        if not check_password_hash(l['phash'],request.form.get('password','')):return login_page('Incorrect password'),403
